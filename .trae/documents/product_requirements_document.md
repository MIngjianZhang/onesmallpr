...
| 角色 | 注册方式 | 核心权限 |
|------|----------|----------|
| 初学者用户 | 模拟登录/注册 | 浏览任务、生成协议、提交PR、查看知识图谱 |
...
| 任务发现页 | 智能推荐 | 从GitHub获取真实的Good First Issue，使用Doubao AI过滤"虚假简单"任务 |
| 任务发现页 | 手动刷新 | 提供刷新按钮获取最新任务列表，1小时缓存机制 |
...
### 用户主流程：
1. 用户通过模拟登录/注册系统创建账户
2. 新用户进入技术画像评估和环境自检流程
3. 系统从GitHub获取真实的Good First Issue任务（过滤掉"虚假简单"任务）
4. 用户可手动刷新获取最新任务列表（1小时缓存过期）
5. 选择任务后进行理解检测问答
6. 通过检测后生成任务协议文档
7. 本地完成任务后提交PR
8. PR成功后更新知识图谱和成就
